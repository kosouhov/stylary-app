		...
		<div class="detailed__content">
			<div class="detailed-content" onclick="event.stopPropagation()">
				...
				<div class="detailed-content__data">
					...
					<div class="detailed-content__price" v-if="price">Стоимость набора {{ price }} Р<span>–</span></div>
					<div class="detailed-content__price" v-if="!price">Базовый набор</div>
				</div>
				<div class="detailed-content__add">
					<div 
						v-if="price"
						:class="['detailed-content__button', page.added ? 'detailed-content__button_remove' : false]" 
						@click="$emit('add', {pageId: id.pageId, catId: id.catId})">{{ page.added ? 'Удалить' : 'Добавить' }}
					</div>
					<div 
						v-if="!price"
						:class="['detailed-content__button', page.added ? 'detailed-content__button_added' : false]" 
						@click="$emit('add', {pageId: id.pageId, catId: id.catId})">{{ page.added ? 'Выбрано' : 'Выбрать' }}
					</div>
				</div>
			</div>
		</div>
